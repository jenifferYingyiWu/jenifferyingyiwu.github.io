...
class MeanReciprocalRank(Evaluation):
    ...
    def load(self, ranked_list, elem):
        ...
        self._ground_truth.append(elem)
    ...
    def load_ground_truth(self, gt):
        raise NotImplementedError("load_ground_truth() method not allowed. User load(ground_truth, query) instead")
...
class AveragePrecision(Evaluation):
    ...
    def __compute(self):
        ...
        for item in self._test:
            try:
                hits = self._ground_truth.index(item) + 1
                hits += 1
                p = hits/float(i)
