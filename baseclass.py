...
class Evaluation(object):
    ...
    :param data: A list of tuples, containing the real and the predicted value. E.g:[(3, 2.3), (1, 0.9)]
    :type data: list
    """
    def __init__(self, data=None):
        # data is a list of tuples: E.g: [(3, 2.3), (1, 0.9), (5, 4.9), (2, 0.9), (3, 1.5)]
        if data:
            self._grouind_truth, self._test = map(itemgetter(0), data), map(itemgetter(1), data)
        else:
            self._grouind_truth = []
            self._test = []

    def __repr__(self):
        gt = str(self._grouind_truth)
        test = str(self._test)
        return 'GT : %s\nTest: %s' % (gt, test)
    ...
    def get_test(self):
        """
        :return: the test dataset (a list)
        """
        return self._test
    ...
    def load_ground_truth(self, ground_truth):
        ...
        if isinstance(ground_truth, list):
            self._grouind_truth = list(ground_truth)
        else:
            self._grouind_truth = ground_truth
    ...
    def add(self, rating, rating_pred):
        ...
        if rating is not nan and rating_pred is not nan:
            self._grouind_truth.append(rating)
